...
Sphere::Sphere(const Vector3& center, double radius, const Vector3RGB& color, double rugosity)
  : center_(center),
    radius_(radius),
    color_(color),
    rugosity_(rugosity)
...
Pixel Sphere::intersect(const LightRay& lr, LightRay& lr2, bool& contact, double& dist) const
{
  ...
  if(orthogDist < radius_ and orthogDist > 0)
  {
    ...
    //Randomize lr2
    double randX = rugosity_*(double(std::rand()%1000)-500);
    double randY = rugosity_*(double(std::rand()%1000)-500);
    double randZ = rugosity_*(double(std::rand()%1000)-500);
    lr2.dir_.x_ += randX;
    lr2.dir_.y_ += randY;
    lr2.dir_.z_ += randZ;
    lr2.dir_.normalize();
